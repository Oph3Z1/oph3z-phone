...
local function playCallPose(ped)
    if loadAnimDict(ANIM.dict) then
        TaskPlayAnim(ped, ANIM.dict, ANIM.call, 3.0, -1.0, -1, 49, 0, false, false, false)
    end
end
...
function Phone.stopAnim()
    if not Config.UseProp then return end
    if Phone.inCallAnim then return end
    local ped = PlayerPedId()
    CreateThread(function()
        if loadAnimDict(ANIM.dict) then
...
function Phone.stopCallAnim()
    if not Phone.inCallAnim then return end
    Phone.inCallAnim = false
    if not Config.UseProp then return end
    local ped = PlayerPedId()
    CreateThread(function()
...
function Phone.open()
    ...
    SendNUIMessage({
        ...
        data = {
            ...
            apps     = Config.Apps,
            walkKey  = Config.WalkKey or 'LMENU',
...
RegisterNetEvent('oph3z-phone:call:incoming', function(data)
    ...
    Phone.call = { callId = data.callId, role = 'callee', state = 'incoming', video = data.video == true }
    callMsg({
...
RegisterNetEvent('oph3z-phone:call:outgoing', function(data)
    if not data then return end
    Phone.call = { callId = data.callId, role = 'caller', state = 'outgoing', video = data.video == true }
    callMsg({ type = 'outgoing', callId = data.callId, number = data.number, name = data.name, img = data.img, video = data.video })
    if not Phone.call.video then Phone.startCallAnim() end
end)
...
RegisterNetEvent('oph3z-phone:call:connected', function(data)
    ...
    if not Phone.call.video and not (Phone.videoCall and Phone.videoCall.active) then
        Phone.startCallAnim()
    end
end)
