...
const CheckIcon = () => (
  <svg width="1em" height="1em" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" aria-hidden>
...
// The share picker: lists nearby players who can receive, and sends the pending
// share payload to whichever one is tapped. Opened by dispatching openShare(...).
export default function AirdropPicker() {
  ...
  // Scan on open, then keep the list fresh while nobody has been picked yet.
  useEffect(() => {
  ...
  // Auto-close shortly after a successful hand-off.
  useEffect(() => {
  ...
  const rowStatus = (person) => {
    ...
    if (sendState === 'accepted') return <span className="ad-person__status is-accepted"><CheckIcon /></span>;
    if (sendState === 'declined') return <span className="ad-person__status is-declined"><XMarkIcon /></span>;
    if (failed) return <span className="ad-person__status is-declined">{t('airdrop.failed')}</span>;
    // sending / waiting -> show "Sent" immediately (no loading spinner).
    return (
    ...
    <>
      ...
      <div className="ad-sheet">
        <div className="ad-sheet__head">
          <span className="ad-sheet__icon"><AirDropGlyph /></span>
          <span className="ad-sheet__title">{t('airdrop.title')}</span>
          <button className="ad-sheet__close" onClick={() => dispatch(closeShare())} aria-label="Close">✕</button>
        </div>

        {nearby.length === 0 ? (
          <div className="ad-empty">
            <div className="ad-empty__glyph"><AirDropGlyph /></div>
            {scanning ? (
