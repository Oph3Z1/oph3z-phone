...
const PrevGlyph = () => (
    <svg viewBox="0 0 24 24" fill="currentColor" width="1em" height="1em">
        <path d="M7 6v12h2V6zm12 0l-8 6 8 6z" />
    </svg>
);
const NextGlyph = () => (
    <svg viewBox="0 0 24 24" fill="currentColor" width="1em" height="1em">
        <path d="M17 6v12h-2V6zM5 6l8 6-8 6z" />
    </svg>
);
const NoteGlyph = () => (
    <svg viewBox="0 0 24 24" fill="currentColor" width="1em" height="1em">
        <path d="M9 17V6l10-2v9" fill="none" stroke="currentColor" strokeWidth="1.7" />
...
function Scrubber({ value, max, onChange, disabled }) {
    ...
    return (
        <div
            className={`cc-scrub${disabled ? ' is-disabled' : ''}`}
            ref={ref}
            onPointerDown={onDown}
            onPointerMove={onMove}
        >
            <div className="cc-scrub__track">
                <div className="cc-scrub__fill" style={{ width: `${pct}%` }} />
            </div>
    ...
    return (
        <div className={`cc${open ? ' is-open' : ''}`} onPointerDown={close}>
            <div className="cc-card cc-toggles" onPointerDown={stop}>
                <button
                    className={`cc-toggle${airplane ? ' is-on cc-toggle--airplane' : ''}`}
                ...
                <button
                    className={`cc-toggle${airdrop ? ' is-on cc-toggle--airdrop' : ''}`}
                ...
                <button
                    className={`cc-toggle${flashlightOn ? ' is-on cc-toggle--flash' : ''}`}
            ...
            <div className="cc-card cc-music" onPointerDown={stop}>
                <div className="cc-music__head">
                    ...
                    <div className="cc-music__meta">
                        <div className="cc-music__title">
                            {hasTrack ? music.title : t('controlCenter.noMusic')}
                        </div>
                        <div className="cc-music__artist">
                            {hasTrack
                                ? music.artist || t('controlCenter.unknownArtist')
                                : t('controlCenter.notPlaying')}
                        </div>
                    </div>
                </div>
                ...
                <div className="cc-music__scrub">
                    ...
                    <span className="cc-music__time cc-music__time--end">
                        {hasTrack && music.duration
                            ? `-${fmtTime(music.duration - music.position)}`
                            : fmtTime(music.duration)}
                    </span>
                </div>

                <div className="cc-music__controls">
                    <button
                        className="cc-music__btn"
                        disabled={!hasTrack}
                        onClick={() => fetchNui('phone:spotify:prev', {}, {})}
                    >
                        <PrevGlyph />
                    </button>
                    <button
                        className="cc-music__btn cc-music__btn--play"
                        disabled={!hasTrack}
                        onClick={() => fetchNui('phone:spotify:toggle', {}, {})}
                    >
                    ...
                    <button
                        className="cc-music__btn"
                        disabled={!hasTrack}
                        onClick={() => fetchNui('phone:spotify:next', {}, {})}
                    >
                        <NextGlyph />
                    </button>
                </div>
