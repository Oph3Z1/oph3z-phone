...
// Phone-wide iOS-style text-input popup. Rendered once inside the phone; driven by
// the prompt slice (openPrompt thunk).
//   - single field  -> confirm resolves the entered string
//   - multiple fields -> confirm resolves an object { [key]: value }
// Cancel (or backdrop / Escape) resolves null.
export default function InputDialog() {
  const dispatch = useDispatch();
  const { open, title, message, placeholder, value, confirmText, cancelText, maxLength, fields } =
    useSelector((s) => s.prompt);

  // Normalise to a field list; single-field uses a sentinel key so both paths share code.
  ...
  // Seed every field each time the popup opens, then focus the first one.
  useEffect(() => {
    if (!open) return;
    const seed = {};
    defs.forEach((f) => {
    ...
    const id = setTimeout(() => {
      ...
      if (el) {
        ...
        el.setSelectionRange(end, end); // caret at end, not select-all
      }
    }, 60);
  ...
  // Confirm is enabled only when every required field has a value.
