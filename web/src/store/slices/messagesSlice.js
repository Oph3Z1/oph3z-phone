...
const messagesSlice = createSlice({
    ...
    reducers: {
        ...
        markReadLocal(state, action) {
            ...
            const th = state.threads.find((t) =>
                t.isGroup ? `g:${t.gid}` === key : t.number === key,
            );
            if (th) th.unread = 0;
        },
...
export const openThread = (number) => async (dispatch) => {
    const t = await fetchNui(
        'phone:messages:open',
        { number },
        { number, name: number, avatar: '', items: [] },
    );
    if (t) {
...
export const sendRequest = (number, amount) => async (dispatch) => {
    const msg = await fetchNui(
        ...
        { to: number, amount },
        {
            ...
            type: 'request',
            body: String(amount),
            meta: { amount, status: 'pending', payer: number, payee: 'me' },
            ts: Math.floor(Date.now() / 1000),
            read: true,
        },
    );
...
export const settleNegotiation = (number, id) => async (dispatch) => {
    const res = await fetchNui(
        'phone:messages:negotiate',
        { action: 'settle', number, id },
        { ok: true },
    );
...
export const deleteThreads = (numbers) => async (dispatch) => {
    dispatch(removeThreadsLocal(numbers));
