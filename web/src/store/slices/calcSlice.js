...
// Browser-dev mock so the history isn't empty via `npm run dev`.
const MOCK = [
...
const initialState = {
  loaded: false,
  history: [], // [{ id, expr, value }] — newest first
};
...
export const addHistory = (expr, value) => async (dispatch) => {
  ...
  dispatch(addHistoryLocal({ id: tmpId, expr, value })); // optimistic
