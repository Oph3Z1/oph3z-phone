...
export const toggleAlarm = (id) => async (dispatch, getState) => {
    ...
    dispatch(setAlarmEnabled({ id, enabled: next }));
...
export const resumeTimer = () => async (dispatch, getState) => {
    ...
    dispatch(
        setTimerLocal({
            ...tm,
            paused: false,
            deadline: Date.now() + Math.max(1, tm.remaining) * 1000,
        }),
    );
...
export const deleteRecent = (index) => async (dispatch, getState) => {
    ...
    await fetchNui('phone:clock:deleteRecent', { index: index + 1 }, true);
};

export const stopRing = () => async (dispatch) => {
    dispatch(setRinging(null));
