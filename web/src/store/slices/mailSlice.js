...
const MOCK = {
    ...
    inbox: [
        {
            id: 5,
            from: 'noreply@lsbank.com',
            fromName: 'LS Bank',
            subject: 'Statement ready',
            body: 'Your monthly statement is now available. Balance: $12,430.',
            system: true,
            read: false,
        ...
        {
            id: 4,
            from: 'mike.reyes@mail.com',
            fromName: 'Mike Reyes',
            subject: 'Tonight?',
            body: 'You free tonight? Thinking about the docks meetup. Bring the photos.',
            read: false,
            ts: Math.floor(Date.now() / 1000) - 9000,
        },
        {
            id: 2,
            from: 'careers@weazelnews.com',
            fromName: 'Weazel News',
            subject: 'Application received',
            body: 'Thanks for applying. We will be in touch.',
            system: true,
            read: true,
            ts: Math.floor(Date.now() / 1000) - 200000,
        },
    ],
    sent: [
        {
            id: 3,
            to: 'mike.reyes@mail.com',
            toName: 'Mike Reyes',
            subject: 'Re: Tonight?',
            body: 'Yeah, count me in. 9pm.',
            ts: Math.floor(Date.now() / 1000) - 8000,
        },
    ],
};
...
export const sendMail = (data) => async (dispatch) => {
    const res = await fetchNui('phone:mail:send', data, {
        ok: true,
        sent: { id: 'tmp' + Date.now(), ...data, ts: Math.floor(Date.now() / 1000) },
    });
...
export const deleteMail = (folder, id) => async (dispatch) => {
    dispatch(removeMailLocal({ folder, id }));
