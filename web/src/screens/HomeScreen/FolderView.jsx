...
export default function FolderView({ folderId, appMap }) {
    ...
    const onMove = (e) => {
        ...
        if (!pr.mode) {
            if (moved) {
                ...
                if (editing && pr.appId) {
                    pr.mode = 'drag';
                    beginDrag(pr.appId, e.clientX, e.clientY);
    ...
    const onUp = (e) => {
        ...
        if (dragRef.current) {
            ...
            if (outside) {
                dispatch(
                    removeFromFolder({ appId, folderId, toKey: `p:${homePage}`, toIndex: 999 }),
                );
                close();
                ...
                let originPage = -1;
                pagesData.forEach((pgArr, pi) => {
                    if (pgArr.includes(appId)) originPage = pi;
                });
                ...
                if (curArr.length === 0) {
                    ...
                    dispatch(
                        moveInFolder({ folderId, appId, toPage: page, toIndex: curArr.length }),
                    );
                }
            }
            dispatch(saveHome());
    ...
    return (
        <div
            className={`hs-folderview${drag ? ' is-grabbing' : ''}`}
            ref={rootRef}
            onPointerDown={close}
        >
            {editing ? (
                <input
                    ...
                    onBlur={(e) => {
                        dispatch(renameFolder({ folderId, name: e.target.value }));
                        dispatch(saveHome());
                    }}
            ...
            {drag && appMap[drag.appId] && (
                <div className="hs-clone" style={{ left: clone.x, top: clone.y }}>
                    <span className="hs-tile__img" style={{ width: '3.4em', height: '3.4em' }}>
                        {appMap[drag.appId].icon ? (
                            <img src={appMap[drag.appId].icon} alt="" />
                        ) : null}
                    </span>
                </div>
            )}
