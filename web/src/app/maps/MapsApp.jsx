...
export default function MapsApp() {
    ...
    const focus = useSelector((s) => s.maps.focus);
    ...
    const blipMarkers = useRef([]);
    const ctrlHeld = useRef(false);
    const blipDragging = useRef(false);
    ...
    useEffect(() => {
        ...
        const onKey = (e) => {
            if (e.key === 'Control') setCtrl(e.type === 'keydown');
        };
    ...
    useNuiEvent('phone:maps:pos', (d) => {
        ...
        if (!playerMarker.current) {
            playerMarker.current = L.marker(ll, { icon: playerIcon, zIndexOffset: 1000 }).addTo(
                map.current,
            );
            map.current.setView(ll, 4);
        } else {
    ...
    useEffect(() => {
        ...
        if (viewOnly) return;
        blips.forEach((b) => {
            const mk = L.marker(toLatLng(b.x, b.y), { icon: blipIcon, draggable: true }).addTo(
                layer,
            );
            ...
            mk.on('dragstart', () => {
                blipDragging.current = true;
            });
            mk.on('dragend', (e) => {
                blipDragging.current = false;
                const ll = e.target.getLatLng();
                dispatch(moveBlip(b.id, ll.lng, ll.lat));
                if (mk.dragging && !ctrlHeld.current) mk.dragging.disable();
            });
    ...
    const recenter = () => {
        ...
        if (lastPos.current && map.current) {
            map.current.setView(toLatLng(lastPos.current.x, lastPos.current.y), 4, {
                animate: true,
            });
        }
    };
    ...
    const savePending = async () => {
        const c = pendingCoords.current;
        if (!c) return;
        await dispatch(
            addBlip({ x: c.x, y: c.y, label: pendingName.trim() || t('maps.savedPlace') }),
        );
        setPending(null);
        setPendingName('');
    };
    ...
    const backToChat = () => {
        if (returnTo) {
            if (String(returnTo).startsWith('g:'))
                dispatch(setResumeGroup(String(returnTo).slice(2)));
            ...
            {pending && (
                <div className="maps__sheet">
                    ...
                    <div className="maps__row">
                        <button
                            className="maps__btn maps__btn--ghost"
