...
// Bottom sheet to add an alarm (hour + minute + optional label).
...
export default function AlarmTab() {
  ...
  // Leaving no alarms while editing would hide the (disabled) Edit/Done button and
  // trap us in edit mode — drop out of it once the list is empty.
  useEffect(() => {
  ...
  return (
    <div className="clk-screen">
      <div className="clk-head">
        ...
        <button className="clk-head__add" onClick={() => setAdding(true)} aria-label={t('clock.addAlarm')}>
          <PlusGlyph />
        </button>
      </div>
            ...
            {sorted.map((a) => (
              <div key={a.id} className="clk-alarm">
                ...
                <div className="clk-alarm__main">
                  <span className={`clk-alarm__time${a.enabled ? '' : ' is-off'}`}>
                    {pad2(a.hour)}:{pad2(a.min)}
                  </span>
                  <span className="clk-alarm__label">{a.label || t('clock.alarm')}</span>
                </div>
                {!editing && (
                  <Switch on={a.enabled} onChange={() => dispatch(toggleAlarm(a.id))} />
                )}
              </div>
            ))}
          </div>
