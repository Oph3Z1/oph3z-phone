...
export default function GroupConversation({ gid, onBack, onInfo }) {
  ...
  const [attach, setAttach] = useState(null); // null | 'menu' | 'gallery' | 'location'
  ...
  useEffect(() => {
    ...
    dispatch(setActive(`g:${gid}`)); // notification suppression while viewing
    ...
    <div className="msg msg--conv">
      ...
      <div className="msg-conv__scroll" ref={scrollRef}>
        {items.map((m, i) => {
          ...
          const showName = !out && (!prev || prev.from !== m.from || prev.type === 'system');
          return (
            <GroupBubble
              key={m.id}
              msg={m}
