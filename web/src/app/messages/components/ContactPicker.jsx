...
// A bottom-sheet list of the player's contacts. Used to pick a contact to share
// into a conversation. `onPick` gets the chosen contact.
export default function ContactPicker({ title, onPick, onClose }) {
  ...
  const list = useMemo(() => {
    ...
    return sorted.filter((c) => c.name.toLowerCase().includes(query) || (c.number || '').includes(query));
  }, [contacts, q]);
    ...
    <>
      ...
      <div className="msg-cpick">
        ...
        <div className="msg-cpick__list">
          {list.length === 0 && <div className="msg-cpick__empty">{t('phone.noContacts')}</div>}
          {list.map((c) => (
