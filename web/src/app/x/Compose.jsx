...
// Detect media type from a URL extension (for the "add link" option).
...
export default function Compose({ me, replyTo, initialMedia, onClose, onPosted }) {
  ...
  const [sheet, setSheet] = useState(null); // photos | gif | emoji | link
  ...
  const insertEmoji = (e) => {
    ...
    if (!el) { setText((v) => (v + e).slice(0, MAX)); return; }
  ...
  const takePhoto = () => {
    // Round-trip through the Camera app; X re-opens the composer with the capture.
    dispatch(setPendingCompose({ replyTo: replyTo ? { id: replyTo.id, author: replyTo.author } : null }));
  ...
  return (
    <div className="x-compose">
      <div className="x-compose__bar">
        <button className="x-compose__cancel" onClick={onClose}>{t('common.cancel')}</button>
            ...
            <div className={`x-compose__media x-media--${media.length}`}>
              {media.map((m, i) => (
                <div key={i} className="x-compose__mediacell">
                  ...
                  <button className="x-compose__mediadel" onClick={() => removeMedia(i)}><CloseIcon size={14} /></button>
                </div>
              ))}
            </div>
          )}
      ...
      <div className="x-compose__toolbar">
        <div className="x-compose__tools">
          <button disabled={media.length >= MAX_MEDIA} onClick={() => setSheet('photos')} aria-label={t('x.addPhoto')}><ImageIcon /></button>
          <button disabled={media.length >= MAX_MEDIA} onClick={takePhoto} aria-label={t('x.takePhoto')}><CameraIcon /></button>
          <button disabled={media.length >= MAX_MEDIA} onClick={() => setSheet('gif')} aria-label="GIF"><GifIcon /></button>
          <button disabled={media.length >= MAX_MEDIA} onClick={() => setSheet('link')} aria-label={t('x.addLink')}><LinkIcon /></button>
          <button onClick={() => setSheet(sheet === 'emoji' ? null : 'emoji')} aria-label={t('x.emoji')}><EmojiIcon /></button>
        </div>
        <span className={`x-compose__count${remaining < 0 ? ' is-over' : ''}${remaining <= 20 ? ' is-low' : ''}`}>{remaining}</span>
      </div>

      {sheet === 'photos' && (
        <PhotoPickerSheet selected={[]} onClose={() => setSheet(null)} onDone={(items) => { addMedia(items); setSheet(null); }} />
      )}
      {sheet === 'gif' && (
        <GifPicker onClose={() => setSheet(null)} onSelect={(url) => { addMedia([{ url, type: 'gif' }]); setSheet(null); }} />
      )}
      {sheet === 'emoji' && (
        <div className="x-compose__emoji"><EmojiPicker onSelect={insertEmoji} onClose={() => setSheet(null)} /></div>
      )}
      {sheet === 'link' && (
        <div className="x-sheet" onClick={() => setSheet(null)}>
          <div className="x-sheet__panel" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="x-sheet__actions">
              <button className="x-sheet__btn" onClick={() => setSheet(null)}>{t('common.cancel')}</button>
              <button className="x-sheet__btn x-sheet__btn--primary" disabled={!/^https?:\/\//i.test(linkUrl.trim())} onClick={addLink}>{t('common.add')}</button>
            </div>
          </div>
        </div>
      )}
