...
// Who reposted / liked a post, with switchable tabs. Opens from the stats row in
// the post detail. Rows reuse the follower-list row (follow/unfollow inline).
export default function EngagersScreen({ postId, initialTab, onBack }) {
  ...
  useEffect(() => {
    let alive = true;
    setItems(null);
    fetchNui('phone:x:postEngagers', { id: postId, type: tab }, { ok: true, accounts: [] })
    ...
    <div className="x-screen">
      ...
      <div className="x-scroll">
        {items === null ? (
          ...
          <div className="x-empty x-empty--sm">{tab === 'reposts' ? t('x.noReposts') : t('x.noLikes')}</div>
        ) : (
