...
// The X in-app "bell": likes, replies, reposts, follows and mentions.
export default function NotificationsScreen({ onBack }) {
  ...
  useEffect(() => {
    fetchNui('phone:x:notifs', {}, { ok: true, items: [] }).then((r) => r && r.ok && setItems(r.items || []));
  }, [liveTick]);

  const open = (n) => {
    if (n.postId && (n.type === 'like' || n.type === 'reply' || n.type === 'repost' || n.type === 'mention')) {
    ...
    <div className="x-screen">
      ...
      <div className="x-scroll">
        {items === null ? (
          ...
          items.map((n) => (
            <button key={n.id} className={`x-notif${n.read ? '' : ' is-unread'}`} onClick={() => open(n)}>
              <span className={`x-notif__ico x-notif__ico--${n.type}`}>{TYPE_ICON[n.type] || TYPE_ICON.like}</span>
