...
export default function MusicApp() {
    ...
    useEffect(() => {
        if (deliver && deliver.appId === 'music') {
            ...
            if (track) {
                fetchNui('phone:spotify:play', { track, queue: [track], index: 1 }, {});
                setNowOpen(true);
            }
        }
    }, [deliver, dispatch]);
    ...
    const shareCurrent = () =>
        setShare(
            current ||
                (music.title
                    ? { title: music.title, artist: music.artist, artwork: music.artwork }
                    : null),
        );
    const go = (nextTab) => {
        setTab(nextTab);
            ...
            <div className="spapp">
                ...
                {music.title && (
                    <div className="sp-mini" onClick={() => setNowOpen(true)}>
                        <span
                            className="sp-mini__art"
                            style={
                                music.artwork ? undefined : { background: gradientFor(music.title) }
                            }
                        >
                            {music.artwork ? (
                        ...
                        <span className="sp-mini__wave">
                            <Waveform size={14} on={music.playing} />
                        </span>
                        <button
                            className="sp-mini__play"
                            onClick={(e) => {
                                e.stopPropagation();
                        ...
                        <span
                            className="sp-mini__bar"
                            style={{
                                width: music.duration
                                    ? `${Math.min(100, (music.position / music.duration) * 100)}%`
                                    : '0%',
                            }}
                        />
                    </div>
                ...
                <div className="sp-nav">
                    <button
                        className={`sp-nav__btn${tab === 'library' && !playlist ? ' is-on' : ''}`}
                        onClick={() => go('library')}
                    >
                        <LibraryIcon size={21} />
                        <span>{t('spotify.library')}</span>
                    </button>
                    <button
                        className={`sp-nav__btn${tab === 'search' && !playlist ? ' is-on' : ''}`}
                        onClick={() => go('search')}
                    >
                        <SearchIcon size={21} />
                        <span>{t('spotify.search')}</span>
                    </button>
                </div>
