...
export default function TrackMenu({ track, playlistId, onShare, onClose }) {
    ...
    const like = async () => {
        ...
        dispatch(
            pushToast({
                title: liked ? t('spotify.removedFromLiked') : t('spotify.addedToLiked'),
                body: '',
            }),
        );
        onClose();
    ...
    return (
        <div className="sp-sheet" onClick={onClose}>
            <div className="sp-sheet__panel" onClick={(e) => e.stopPropagation()}>
                ...
                <Header />
                <button className="sp-item" onClick={like}>
                    <HeartIcon size={19} filled={liked} />{' '}
                    {liked ? t('spotify.removeFromLiked') : t('spotify.addToLiked')}
                </button>
                <button className="sp-item" onClick={() => setAdding(true)}>
                    <PlusIcon size={19} /> {t('spotify.addToPlaylist')}
                </button>
                <button
                    className="sp-item"
                    onClick={() => {
                ...
                >
                    <ShareIcon size={19} /> {t('spotify.share')}
                </button>
                {playlistId && playlistId !== 'liked' && (
                    <button className="sp-item sp-item--danger" onClick={remove}>
                        <TrashIcon size={19} /> {t('spotify.removeFromPlaylist')}
                    </button>
                )}
            </div>
        </div>
