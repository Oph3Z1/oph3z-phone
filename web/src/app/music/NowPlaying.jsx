...
import {
  ChevronDown, ShareIcon, PlayIcon, PauseIcon, NextIcon, PrevIcon,
  HeartIcon, PlusIcon, SpeakerIcon, HeadphoneIcon, VolumeIcon, NoteIcon,
} from './icons';

// Full-bleed immersive player: the artwork fills the screen (blurred behind a big
// sharp cover); everything floats over a bottom scrim. Volume is synced to the
// phone's media volume; the speaker toggle broadcasts to nearby players.
export default function NowPlaying({ onClose, onShare }) {
  ...
  const addTo = async (pid) => {
    if (!current) return;
    const r = await fetchNui('phone:spotify:addTrack', { playlistId: pid, track: current }, { ok: true });
  ...
  return (
    <div className="sp-now">
      <div className="sp-now__bg" style={art ? { backgroundImage: `url(${art})` } : { background: gradientFor(music.title) }} />
      <div className="sp-now__scrim" />

      <div className="sp-now__bar">
        <button className="sp-round sp-round--dark" onClick={onClose}><ChevronDown size={22} /></button>
        <span className="sp-now__ctx">{t('spotify.nowPlaying')}</span>
        <button className="sp-round sp-round--dark" onClick={onShare}><ShareIcon size={19} /></button>
      </div>

      <div className={`sp-now__cover${music.playing ? ' is-playing' : ''}`} style={art ? { backgroundImage: `url(${art})` } : { background: gradientFor(music.title) }}>
      ...
      <div className="sp-now__panel">
        <div className="sp-now__head">
          ...
          <button className="sp-heart" onClick={() => setAdding(true)} disabled={!current} aria-label={t('spotify.addToPlaylist')}><PlusIcon size={24} /></button>
        ...
        <div className="sp-now__foot">
          {allowNearby ? (
            <button className={`sp-chip${nearby ? ' is-on' : ''}`} onClick={toggleNearby}>
              {nearby ? <SpeakerIcon size={18} /> : <HeadphoneIcon size={18} />}<span>{nearby ? t('spotify.speaker') : t('spotify.private')}</span>
            </button>
          ) : <span />}
      ...
      {adding && (
        <div className="sp-sheet" onClick={() => setAdding(false)}>
          <div className="sp-sheet__panel" onClick={(e) => e.stopPropagation()}>
            <button className="sp-sheet__griphit" onClick={() => setAdding(false)} aria-label={t('common.close')}><span className="sp-sheet__grip" /></button>
