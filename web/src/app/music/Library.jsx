    ...
    return (
        <div className="sp-screen">
            ...
            <button className="sp-hero" onClick={() => onOpenPlaylist({ id: 'liked' })}>
                <span
                    className="sp-hero__art"
                    style={likedArt ? { backgroundImage: `url(${likedArt})` } : undefined}
                >
                    <span className="sp-hero__heart">
                        <HeartIcon size={30} filled />
                    </span>
                </span>
                <span className="sp-hero__body">
                    <span className="sp-hero__title">{t('spotify.likedSongs')}</span>
                    <span className="sp-hero__sub">
                        {t('spotify.songCount', { count: liked.length })}
                    </span>
                </span>
                    ...
                    {playlists.map((p) => {
                        ...
                        return (
                            ...
                            >
                                ...
                                <span className="sp-tile__sub">
                                    {t('spotify.songCount', { count: (p.tracks || []).length })}
                                </span>
                            </button>
                        );
                    })}
                    <button className="sp-tile sp-tile--add" onClick={sp.openCreate}>
                        <span className="sp-tile__art sp-tile__art--add">
                            <PlusIcon size={28} />
                        </span>
