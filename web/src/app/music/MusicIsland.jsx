...
export default function MusicIsland() {
    ...
    const hidden =
        !music.title || activeApp === 'music' || locked || inCall || (!music.playing && !expanded);
    useEffect(() => {
    ...
    const openApp_ = () => {
        dispatch(unlock());
        dispatch(openApp('music'));
    };
    const toggle = (e) => {
        e.stopPropagation();
        fetchNui('phone:spotify:toggle', {}, {});
    };
    const next = (e) => {
        e.stopPropagation();
        fetchNui('phone:spotify:next', {}, {});
    };
    const prev = (e) => {
        e.stopPropagation();
        fetchNui('phone:spotify:prev', {}, {});
    };
    const pct = music.duration ? Math.min(100, (music.position / music.duration) * 100) : 0;

    const Art = ({ size }) => (
        <span
            className="sp-isl__art"
            style={{
                width: size,
                height: size,
                ...(music.artwork ? {} : { background: gradientFor(music.title) }),
            }}
        >
            {music.artwork ? (
                <img src={music.artwork} alt="" />
            ) : (
                <NoteIcon size={parseFloat(size) * 8} />
            )}
        </span>
    );
    ...
    if (!expanded) {
        return (
            <div className="sp-isl sp-isl--compact" onClick={() => setExpanded(true)}>
                <Art size="1.5em" />
                <span className="sp-isl__wave">
                    <Waveform size={12} on={music.playing} />
                </span>
            </div>
        );
    }
        ...
        <>
            ...
            <div className="sp-isl sp-isl--expanded">
                <div className="sp-isl__top" onClick={openApp_}>
                    ...
                    <span className="sp-isl__wave sp-isl__wave--big">
                        <Waveform size={24} on={music.playing} />
                    </span>
                </div>

                <div className="sp-isl__track">
                    <span className="sp-isl__fill" style={{ width: `${pct}%` }} />
                </div>
                <div className="sp-isl__times">
                    <span>{fmtTime(music.position)}</span>
                ...
                <div className="sp-isl__ctl">
                    ...
                    <button className="sp-isl__play" onClick={toggle}>
