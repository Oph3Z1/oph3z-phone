...
export default function ExternalShareSheet({ app, item, onClose }) {
    ...
    const toContact = (contact) => {
        ...
        dispatch(
            pushToast({
                title: t('common.sharedWith', { name: contact.name }),
                body: '',
                app: app.id,
            }),
        );
    ...
    return (
        <div className="ext-sheet" onClick={onClose}>
            <div className="ext-sheet__panel" onClick={(e) => e.stopPropagation()}>
                ...
                <button className="ext-sheet__btn" onClick={doAirdrop}>
                    <ShareGlyph /> {t('common.shareAirdrop')}
                </button>
                <button className="ext-sheet__btn" onClick={() => setPicking(true)}>
                    <MsgGlyph /> {t('common.shareMessages')}
                </button>
                <button className="ext-sheet__btn ext-sheet__btn--cancel" onClick={onClose}>
                    {t('common.cancel')}
                </button>
            </div>
        </div>
