...
export default function ContactsView({ onOpen, onAdd }) {
  ...
  // Share my own contact to a nearby player (AirDrop).
  const shareMyCard = () =>
    dispatch(openShare({ kind: 'contact', contact: { name: identity.name || t('phone.myCard'), number: myNumber, img: identity.avatar } }));
  ...
  return (
    <>
      ...
      {/* My card — tap the share icon to AirDrop your own contact to someone nearby. */}
      <div className="pa-mycard">
        ...
        <button className="pa-mycard__share" onClick={shareMyCard} aria-label="Share my contact">
          <ShareGlyph />
        </button>
      </div>
