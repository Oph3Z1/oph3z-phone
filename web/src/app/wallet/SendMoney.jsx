...
export default function SendMoney({ onClose, onSent, prefill }) {
    ...
    const [recipient, setRecipient] = useState(prefill || null);
    ...
    const canSend =
        !!recipient &&
        (recipient.serverId != null || digitsOf(recipient.number || '')) &&
        amt > 0 &&
        amt <= balance &&
        !sending;
    ...
    const submit = async () => {
        ...
        if (res && res.ok) {
            dispatch(
                pushToast({
                    app: 'wallet',
                    title: t('wallet.sentToast', { amount: fmtMoney(amt) }),
                }),
            );
            onSent();
        } else {
            const reason =
                res && res.reason === 'notfound'
                    ? t('wallet.noSuchNumber')
                    : res && res.reason === 'funds'
    ...
    return (
        <div className="walletapp">
                ...
                {recipient ? (
                    <div className="wl-chip">
                        <span className="wl-chip__av">
                            {recipient.serverId != null
                                ? '#'
                                : initialOf(recipient.name || recipient.number)}
                        </span>
                        ...
                        <button className="wl-chip__x" onClick={() => setRecipient(null)}>
                            ✕
                        </button>
                    </div>
                ) : (
                    <>
                        <div className="wl-torow">
                            ...
                            <button
                                className="wl-pickbtn"
                                onClick={() => setPicking(true)}
                                aria-label={t('wallet.chooseContact')}
                            >
                                <PeopleIcon />
                            </button>
                        </div>
                            ...
                            <div className="wl-suggest">
                                {rawDigits.length >= 4 && (
                                    <button
                                        ...
                                        onClick={() => {
                                            setRecipient({ number: rawDigits, name: null });
                                            setQuery('');
                                        }}
                                    >
                                        <span className="wl-suggest__av wl-suggest__av--num">
                                            #
                                        </span>
                                        <div className="wl-suggest__txt">
                                            <span className="wl-suggest__name">
                                                {t('wallet.sendToNumber')}
                                            </span>
                                ...
                                {rawDigits.length <= 5 && (
                                    <button
                                        className="wl-suggest__row"
                                        onClick={() => {
                                            setRecipient({ serverId: Number(rawDigits) });
                                            setQuery('');
                                        }}
                                    >
                                        <span className="wl-suggest__av wl-suggest__av--id">
                                            <IdIcon />
                                        </span>
                                        <div className="wl-suggest__txt">
                                            <span className="wl-suggest__name">
                                            ...
                                            <span className="wl-suggest__num">
                                                {t('wallet.serverId')} {rawDigits}
                                            </span>
                                        </div>
                                    </button>
                                )}
            ...
            <div className="wl-foot">
                <button
                    className={`wl-send${canSend ? '' : ' is-off'}`}
                    onClick={submit}
                    disabled={!canSend}
                >
