...
export default function MarketApp() {
  ...
  // Returning from the Camera: the app remounted (stack reset to feed), so reopen
  // the composer — the in-progress draft is preserved in redux.
  useEffect(() => {
    if (reopenCompose) {
  ...
  const nav = useMemo(() => ({
    ...
    openCompose: () => { dispatch(startDraft({ mode: 'new' })); push({ name: 'compose' }); },
    openEdit: (l) => {
      dispatch(startDraft({
        mode: 'edit', id: l.id, category: l.category, title: l.title, desc: l.desc,
        price: l.price ? String(l.price) : '', media: (l.media || []).map((m) => ({ ...m })),
        allowCalls: l.allowCalls, allowMsg: l.allowMsg,
      }));
      push({ name: 'compose' });
    },
  }), [push, dispatch]);
  ...
  if (view.name === 'feed') {
    ...
    screen = <NewPost onBack={back} onPosted={() => { back(); bumpReload(); }} />;
