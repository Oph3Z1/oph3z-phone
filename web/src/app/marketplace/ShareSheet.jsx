...
// Share a listing or a seller profile over AirDrop (nearby) or into Messages.
// On the receiver, tapping it opens Marketplace to that listing / profile.
...
export default function ShareSheet({ kind, listing, seller, onClose }) {
  ...
  const subtitle = isListing ? fmtPrice(listing.price, t) : t('market.adsCount', { count: seller.count || 0 });
  ...
  return (
    <div className="mkt-sheet" onClick={onClose}>
      <div className="mkt-sheet__panel" onClick={(e) => e.stopPropagation()}>
        <div className="mkt-sheet__title">{isListing ? t('market.shareListing') : t('market.shareProfile')}</div>

        <div className="mkt-share__item">
          {isListing ? (
            <span className="mkt-share__thumb" style={image ? { backgroundImage: `url(${image})` } : undefined}>
              {!image && <span className="mkt-share__ph">{CATEGORY_ICON[listing.category] || '📦'}</span>}
            </span>
          ) : (
        ...
        <button className="mkt-sheet__btn" onClick={doAirdrop}><ShareIcon size={20} /> {t('market.shareAirdrop')}</button>
        <button className="mkt-sheet__btn" onClick={() => setPicking(true)}><MessageIcon size={20} /> {t('market.shareMessages')}</button>
        <button className="mkt-sheet__btn mkt-sheet__btn--cancel" onClick={onClose}>{t('common.cancel')}</button>
      </div>
    </div>
  );
}
