...
export default function Profile({ cid, reloadToken, onBack }) {
    ...
    useEffect(() => {
        ...
        setData(null);
        setMissing(false);
        fetchNui('phone:market:profile', { cid: cid || undefined }, { ok: false }).then((r) => {
            if (!alive) return;
            if (r && r.ok) setData(r);
    ...
    return (
        <div className="mkt-screen">
                ...
                {missing ? (
                    ...
                    <>
                        <div className="mkt-pcard">
                            ...
                            <div className="mkt-pcard__name">
                                {seller.name}
                                {seller.isMe ? (
                                    <span className="mkt-pcard__you">{t('market.you')}</span>
                                ) : null}
                            </div>
                            {seller.number ? (
                                <div className="mkt-pcard__num">{seller.number}</div>
                            ) : null}
                            <div className="mkt-pcard__count">
                                {t('market.adsCount', { count: listings.length })}
                            </div>
                        </div>

                        {listings.length === 0 ? (
                            <div className="mkt-empty mkt-empty--sm">
                                {seller.isMe ? t('market.youNoAds') : t('market.sellerNoAds')}
                            </div>
                        ) : (
            ...
            {sharing && seller && (
                <ShareSheet
                    kind="profile"
                    seller={{ ...seller, count: listings.length }}
