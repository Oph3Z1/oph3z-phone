...
// Search ads by title, description or seller name.
export default function SearchScreen({ onBack }) {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    const id = setTimeout(() => {
      fetchNui('phone:market:feed', { category: 'all', q: query }, { ok: true, listings: [] })
        .then((r) => { if (alive && r && r.ok) { setItems(r.listings || []); setLoading(false); } });
    }, 220);
    ...
    <div className="mkt-screen">
      <div className="mkt-topbar mkt-topbar--search">
        ...
        <div className="mkt-searchbar">
          <SearchIcon size={17} />
          <input autoFocus value={q} placeholder={t('market.searchPh')} onChange={(e) => setQ(e.target.value)} />
          {q ? <button className="mkt-searchbar__clr" onClick={() => setQ('')}><CloseIcon size={15} /></button> : null}
        </div>
      </div>
