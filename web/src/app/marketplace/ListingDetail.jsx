...
export default function ListingDetail({ id, onBack, onChanged }) {
    ...
    useEffect(() => {
        let alive = true;
        fetchNui('phone:market:listing', { id }, { ok: false }).then((r) => {
            if (!alive) return;
            if (r && r.ok) setL(r.listing);
    ...
    const call = () => {
        if (l.seller.number) fetchNui('phone:call:start', { number: l.seller.number }, {});
    };
    const message = () => {
            ...
            <div className="mkt-scroll mkt-detail">
                ...
                <div className="mkt-detail__pad">
                    <div className="mkt-detail__toprow">
                        <span className="mkt-detail__cat">
                            {CATEGORY_ICON[l.category]} {t(`market.cat_${l.category}`)}
                        </span>
                    ...
                    <button className="mkt-seller" onClick={() => nav.openProfile(l.seller.cid)}>
                        ...
                        <div className="mkt-seller__id">
                            ...
                            {l.seller.number ? (
                                <span className="mkt-seller__num">{l.seller.number}</span>
                            ) : null}
                        </div>
                    ...
                    {l.isMine ? (
                        <div className="mkt-detail__owner">
                            <button
                                ...
                                onClick={() => nav.openEdit(l)}
                            >
                                <EditIcon size={18} /> {t('market.edit')}
                            </button>
                            <button className="mkt-btn mkt-btn--danger" onClick={remove}>
                                <TrashIcon size={18} /> {t('market.delete')}
                            </button>
                        </div>
                    ) : (
                        <div className="mkt-detail__contact">
                            {l.allowCalls && (
                                <button className="mkt-btn mkt-btn--solid" onClick={call}>
                                    <PhoneIcon size={18} /> {t('market.call')}
                                </button>
                            )}
                            {l.allowMsg && (
                                <button className="mkt-btn mkt-btn--ghost" onClick={message}>
                                    <MessageIcon size={18} /> {t('market.message')}
                                </button>
                            )}
                        </div>
                    )}
...
function Topbar({ t, onBack, onShare }) {
    return (
        <div className="mkt-topbar">
            ...
            {onShare ? (
                <button className="mkt-iconbtn" onClick={onShare} aria-label={t('market.share')}>
                    <ShareIcon size={18} />
                </button>
            ) : (
