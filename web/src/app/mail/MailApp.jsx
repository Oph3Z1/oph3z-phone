...
export default function MailApp() {
  ...
  // view: { name: 'list' } | { name: 'detail', mail, folder } | { name: 'compose', draft }
  const [view, setView] = useState({ name: 'list' });

  useEffect(() => {
    if (!loaded) dispatch(loadMail());
    dispatch(clearNotifsFor({ app: 'mail' })); // opening the mailbox clears its notifications
  }, [loaded, dispatch]);

  // A mail notification asked to open a specific inbox mail. If it isn't loaded
  // yet, this re-runs when the inbox arrives (focusId is kept until found).
  useEffect(() => {
    if (!focusId) return;
  ...
  if (view.name === 'detail') {
    return (
      <MailDetail
        ...
        onReply={(m) => setView({ name: 'compose', draft: { to: m.from, subject: replySubject(m.subject) } })}
  ...
  if (view.name === 'compose') {
    return (
      <MailCompose
        ...
        onSent={() => { setFolder('sent'); setView({ name: 'list' }); }}
      />
    );
  }
