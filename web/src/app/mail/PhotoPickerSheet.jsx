...
// Bottom sheet to pick photo/video attachments from the Photos app (max 6).
    ...
    <div className="mail-sheet" onClick={onClose}>
      <div className="mail-sheet__panel" onClick={(e) => e.stopPropagation()}>
        <div className="mail-sheet__bar">
          <button className="mail-sheet__cancel" onClick={onClose}>{t('common.cancel')}</button>
          <span className="mail-sheet__title">{t('mail.attach')}</span>
          <button className="mail-sheet__done" onClick={done}>{t('common.done')}</button>
        </div>
        ...
        {media.length === 0 ? (
          ...
          <div className="mail-sheet__grid">
            {media.map((p) => (
              ...
              >
                {p.type === 'video' ? (
                  <video className="mail-sheet__media" src={p.url} muted playsInline preload="metadata" />
                ) : (
                  <img className="mail-sheet__media" src={p.thumb || p.url} alt="" />
                )}
