...
// Compose / reply. `draft` may prefill { to, subject }.
export default function MailCompose({ draft, onClose, onSent }) {
  ...
  // Address suggestions from people you've corresponded with.
  const known = useMemo(() => {
    const map = new Map();
    inbox.forEach((m) => m.from && m.from.includes('@') && map.set(m.from, m.fromName || m.from));
    sent.forEach((m) => m.to && map.set(m.to, m.toName || m.to));
    return [...map.entries()].map(([addr, name]) => ({ addr, name }));
  }, [inbox, sent]);

  const suggestions = to.trim() && !isEmail(to)
    ? known.filter((k) => k.addr.includes(to.toLowerCase()) || k.name.toLowerCase().includes(to.toLowerCase())).slice(0, 4)
    : [];
  ...
  const submit = async () => {
    ...
    setSending(true);
    const res = await dispatch(sendMail({ to: to.trim(), subject: subject.trim(), body, attachments }));
    setSending(false);
    if (res && res.ok) {
      dispatch(pushToast({ type: 'success', title: t('mail.sent_toast') }));
      onSent();
    } else {
      const reason = res && res.reason === 'notfound' ? t('mail.noSuchAddress') : t('mail.sendFailed');
    ...
    <div className="mailapp mail-compose">
          ...
          <div className="mail-compose__atts">
            {attachments.map((a) => (
              <div key={a.url} className="mail-compose__att">
                ...
                <button className="mail-compose__attx" onClick={() => removeAtt(a.url)}>
                  <CloseIcon />
                </button>
              </div>
            ))}
      ...
      {picking && (
        <PhotoPickerSheet
          ...
          onDone={(list) => { setAttachments(list); setPicking(false); }}
        />
      )}
    </div>
  );
