...
// A single keypad button (hoisted so it isn't remounted on every keystroke).
...
// ---- Calculator engine (pure reducer) ------------------------------------
const INIT = { current: '0', previous: null, op: null, overwrite: false, lastExpr: '', error: false };
...
function reducer(state, action) {
  switch (action.type) {
    case 'digit': {
      ...
      if (cur.replace('-', '').replace('.', '').length > 12) return state; // digit cap
    ...
    case 'negate': {
      ...
      const cur = state.current.startsWith('-') ? state.current.slice(1) : '-' + state.current;
      return { ...state, current: cur };
    }
    case 'percent': {
      if (state.error) return state;
      return { ...state, current: stripNum(parseFloat(state.current) / 100), overwrite: false, lastExpr: '' };
    }
    case 'clearEntry':
    ...
    case 'result': // = , or inserting a history value
      return {
        ...INIT,
...
export default function CalculatorApp() {
  ...
  // Keep the receipt feed pinned to the newest entry / current line.
  useLayoutEffect(() => {
  ...
  // Physical keyboard support (phone is focused while open).
  useEffect(() => {
  ...
  return (
    <div className="calcapp">
      <div className="calc__screen">
        <button className="calc__histbtn" onClick={() => setView('history')} aria-label="History">
          <HistoryIcon />
        </button>

        <div className="calc__feed" ref={feedRef}>
          {[...history].reverse().map((h) => (
            <button key={h.id} className="calc__feeditem" onClick={() => insertResult(h.value)}>
      ...
      <div className="calc__pad">
        <Key cls="calc__key--fn" onClick={() => dispatch({ type: clearLabel === 'C' ? 'clearEntry' : 'allClear' })}>
          {clearLabel}
        </Key>
        <Key cls="calc__key--fn" onClick={() => dispatch({ type: 'negate' })}>+/-</Key>
        <Key cls="calc__key--fn" onClick={() => dispatch({ type: 'percent' })}>%</Key>
        <Key cls="calc__key--op" active={opActive('÷')} onClick={() => dispatch({ type: 'op', op: '÷' })}>÷</Key>

        {['7', '8', '9'].map((n) => (
          <Key key={n} cls="calc__key--num" onClick={() => dispatch({ type: 'digit', d: n })}>{n}</Key>
        ))}
        <Key cls="calc__key--op" active={opActive('×')} onClick={() => dispatch({ type: 'op', op: '×' })}>×</Key>
        ...
        <Key cls="calc__key--num calc__key--zero" onClick={() => dispatch({ type: 'digit', d: '0' })}>0</Key>
        <Key cls="calc__key--num" onClick={() => dispatch({ type: 'dot' })}>.</Key>
        <Key cls="calc__key--op" onClick={equals}>=</Key>
      </div>
    </div>
  );
}
