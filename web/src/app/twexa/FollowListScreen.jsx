...
export function UserRow({ acc, canRemove, onRemove }) {
    ...
    return (
        <button
            className="x-userrow"
            onClick={() => nav.openProfile({ id: acc.id, handle: acc.handle })}
        >
            <Avatar account={acc} size="2.7em" />
            <div className="x-userrow__mid">
                <span className="x-userrow__name">
                    {acc.name}
                    <Verified size={13} tier={acc.badge} />
                </span>
                <span className="x-userrow__handle">@{acc.handle}</span>
                {acc.bio ? <span className="x-userrow__bio">{acc.bio}</span> : null}
            </div>
            {canRemove ? (
                <span className="x-btn x-btn--sm x-btn--outline" role="button" onClick={remove}>
                    {t('x.remove')}
                </span>
            ) : !acc.isMe ? (
                <span
...
export default function FollowListScreen({ target, initialTab, isSelf, me, onBack }) {
    ...
    useEffect(() => {
        ...
        fetchNui(
            'phone:x:followList',
            { id: target.id, handle: target.handle, type: tab },
            { ok: true, accounts: [] },
        ).then((r) => {
            ...
            <div className="x-scroll">
                {items === null ? (
                    ...
                    <div className="x-empty x-empty--sm">
                        {tab === 'followers' ? t('x.noFollowers') : t('x.noFollowing')}
                    </div>
                ) : (
                    items.map((a) => (
                        <UserRow
                            key={a.id}
                            acc={a}
                            canRemove={isSelf && tab === 'followers'}
