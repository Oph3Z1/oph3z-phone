...
export default function PostDetail({ id, me, reloadToken, onBack }) {
    ...
    const load = async () => {
        const res = await fetchNui(
            'phone:x:thread',
            { id },
            { ok: true, post: null, ancestors: [], replies: [] },
        );
    ...
    return (
        <div className="x-screen x-detail">
                    ...
                    <>
                        {data.ancestors &&
                            data.ancestors.map((a) => (
                                <div className="x-detail__ancestor" key={a.id}>
                                    <PostCard post={a} me={me} onChanged={load} />
                                </div>
                            ))}
                                ...
                                <div className="x-detail__stats">
                                    ...
                                    {post.likeCount > 0 && (
                                        <button
                                            className="x-detail__stat"
                                            onClick={() =>
                                                nav.openEngagers({ postId: post.id, tab: 'likes' })
                                            }
                                        >
                                            <b>{fmtCount(post.likeCount)}</b>{' '}
                                            {t(post.likeCount === 1 ? 'x.likeOne' : 'x.likeMany')}
                                        </button>
                                    )}
                                </div>
                            )}
                        ...
                        <div className="x-detail__replies">
                            {data.replies && data.replies.length > 0 ? (
                                data.replies.map((r) => (
                                    <PostCard key={r.id} post={r} me={me} onChanged={load} />
                                ))
                            ) : (
            ...
            <div className="x-replybar">
                ...
                />
                <button className="x-replybar__emoji" onClick={() => setEmoji((v) => !v)}>
                    <EmojiIcon size={20} />
                </button>
                <button
                    className="x-replybar__send"
            ...
            {emoji && (
                <div className="x-detail__emoji">
                    <EmojiPicker
                        onSelect={(e) => setReply((v) => v + e)}
