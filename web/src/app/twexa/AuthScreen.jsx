...
export default function AuthScreen() {
    ...
    const submit = async () => {
        ...
        if (mode === 'register') {
            ...
            const res = await fetchNui(
                'phone:x:register',
                { handle: h, name: name.trim(), password, email: addr },
                { ok: true, pending: true, email: addr },
            );
            setBusy(false);
            if (res && res.ok && res.pending) {
                dispatch(setPendingAuth({ mode: 'register', email: res.email }));
                return;
            }
        ...
        const res = await fetchNui(
            'phone:x:login',
            { handle: h, password },
            { ok: true, me: { id: 1, handle: h || 'you', name: 'You', verified: false } },
        );
        setBusy(false);
        if (res && res.ok) {
    ...
    const forgot = async () => {
        ...
        setBusy(true);
        const res = await fetchNui(
            'phone:x:recoverStart',
            { handle: h },
            { ok: true, pending: true, email: 'y**@mail.com' },
        );
        setBusy(false);
        if (res && res.ok && res.pending) {
            dispatch(setPendingAuth({ mode: 'recover', email: res.email }));
            return;
        }
        setErr(t(`x.err_${(res && res.reason) || 'bad'}`) || t('x.errGeneric'));
    };
    ...
    return (
        <div className="x-auth">
            ...
            <h1 className="x-auth__title">
                {mode === 'register' ? t('x.createAccount') : t('x.signIn')}
            </h1>
            ...
            <div className="x-auth__form">
                ...
                {mode === 'login' && (
                    <button className="x-auth__forgot" disabled={busy} onClick={forgot}>
                        {t('x.forgotPassword')}
                    </button>
                )}
            </div>
