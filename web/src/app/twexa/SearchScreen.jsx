...
export default function SearchScreen({ onBack }) {
    ...
    useEffect(() => {
        fetchNui('phone:x:topics', {}, { ok: true, topics: [] }).then(
            (r) => r && r.ok && setTopics(r.topics || []),
        );
    }, []);
            ...
            <div className="x-topbar">
                ...
                <div className="x-search__bar">
                    ...
                    <input
                        autoFocus
                        value={q}
                        placeholder={t('x.searchProfiles')}
                    ...
                    <>
                        ...
                        {topics.length === 0 ? (
                            ...
                            topics.map((tp) => (
                                <button
                                    key={tp.tag}
                                    className="x-topicrow"
                                    onClick={() => nav.openHashtag(tp.tag)}
                                >
                                    <span className="x-topicrow__tag">#{tp.tag}</span>
                                    <span className="x-topicrow__count">
                                        {fmtCount(tp.count)} {t('x.posts')}
                                    </span>
                                </button>
                            ))
                        )}
